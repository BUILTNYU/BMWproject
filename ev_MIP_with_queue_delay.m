...
s2=zeros(3,1);
 ...
 temp=[zeros(2,N*N*G*G),temp,zeros(2,72)];
 A=[A;temp];
 
 % capacity constraints
 
 n=1;
 temp=zeros(24,N*N*G*G+reloc_paths+72);
for h=1:G
    for j=1:N
        for g=1:G
            for i=1:N 
                temp(j+(h-1)*N,(h-1)*N*G*N+(j-1)*G*N+(g-1)*N+i)=1;
                temp(j+(h-1)*N,N*N*G*G+reloc_paths+n)=-rho(1)*m;
                temp(j+(h-1)*N,N*N*G*G+reloc_paths+n+1)=-(rho(3)-rho(2))*m;
                temp(j+(h-1)*N,N*N*G*G+reloc_paths+n+2)=-(rho(3)-rho(2))*m;
            end
        end
        n=n+3;
    end
