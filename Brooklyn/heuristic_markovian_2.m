load Lamda6
dist_h=20; % time between two charge neighbor nodes
grid_dist=5; % distance between two neighbor nodes
set_O=[3 7 14]'; % origin nodes of vehicles
theta = 0.02; % objective function weight of rebalanced distance 
mu = 80*6*N*ones(NN1,1)/20; % service rate : 1 customer per 10 minutes
charg_stations=[2 6];
[adj,dist_ij]=graph(layers,nodes,charg_stations,grid_dist,dist_h)
function [RebalancePath,RebalanceDecision]=heuristic(Lamda,layers,nodes, initial_loc,station_capacity,dist_tij,adj,mu,theta)
